...
public class DemoSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		auth.inMemoryAuthentication()
			.withUser(users.username("reza").password("test123").roles("EMPLOYEE"))
			.withUser(users.username("kapil").password("test123").roles("EMPLOYEE", "MANAGER"))
			.withUser(users.username("ruhul").password("test123").roles("ADMIN", "EMPLOYEE"));
	}
